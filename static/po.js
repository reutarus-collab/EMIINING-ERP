...
async function loadInventoryForPO() {
...
async function loadPOs() {
    ...
    poListCache = await res.json();
    const tbody = document.getElementById('po-list-body');
    if(!tbody) return;
    tbody.innerHTML = '';
    if(poListCache.length === 0) {
...
function renderPOCart() {
    ...
    tbody.innerHTML = '';
    let total = 0;
    currentPoCart.forEach((item, idx) => {
        ...
        tbody.innerHTML += `
            <tr>
                ...
                <td><button class="btn-sm btn-danger" onclick="currentPoCart.splice(${idx}, 1); renderPOCart()">x</button></td>
            </tr>
        `;
    });
...
async function submitPO() {
    ...
    if(currentPoCart.length === 0) return alert("Add at least one item to the order.");
...
async function openReceiveGRN(poId) {
    // In a real app, you'd fetch the specific PO lines from the backend.
    // For now, we will simulate fetching the PO lines to build the GRN form.
    const po = poListCache.find(p => p.id === poId);
    if(!po) return;
    
    // Fetch PO details (You need an endpoint for this, we will add it to routes/po.py shortly, or assume it returns the lines)
    const res = await fetch(`/api/po/${poId}/lines`);
    const poData = await res.json();
    
    currentReceivingPO = poData;
    ...
    poData.lines.forEach((line, idx) => {
        // Only show lines that aren't fully received yet
        const remaining = line.qty_ordered - line.qty_received_so_far;
        if (remaining <= 0) return;

        tbody.innerHTML += `
            <tr data-polineid="${line.id}" data-ordered="${remaining}">
                <td>${line.item_name}<br/><small>Ordered: ${line.qty_ordered} | Pending: ${remaining}</small></td>
                <td><input type="number" class="grn-recv-qty" max="${remaining}" placeholder="Qty Rcvd" style="width:80px;" oninput="calcAccepted(${idx})"></td>
                <td><input type="number" class="grn-rej-qty" value="0" min="0" style="width:80px;" oninput="calcAccepted(${idx})"></td>
                <td><span class="grn-acc-qty" style="font-weight:bold; color:green;">0</span></td>
                <td><input type="text" class="grn-batch" placeholder="Lot/Batch" style="width:100px;"></td>
            </tr>
        `;
    });
...
async function submitGRN() {
    ...
    try {
        const res = await fetch(`/api/po/${currentReceivingPO.id}/receive`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify(payload)
        });
